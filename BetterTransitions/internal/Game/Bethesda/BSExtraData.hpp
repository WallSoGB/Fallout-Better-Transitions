...
public:
	...
	static BSExtraData* Create(uint8_t ucType, uint32_t uiSize, uint32_t vtbl);
...
public:
	...
	union
	{
		class NPCPerksInfo* pPerksInfo;
		class TESAmmo*		pAmmo;		//	For Projectile refs
	};
#endif

	bool HasExtra(uint32_t type) const;

	BSExtraData* GetExtraData(uint8_t acType) const;

	template <class T>
	__forceinline T* GetExtraData() const {
		return (T*)GetExtraData(T::kExtraDataType);
	}

	void RemoveExtra(BSExtraData* apExtra, bool abDelete);

	void RemoveExtra(uint32_t acType);
